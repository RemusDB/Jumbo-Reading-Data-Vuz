# Part: Analyzing Jumbo's main metrics
id: load-transform-data
url: load-transform-data

## Description

Quickly interpret Jumbo's metrics with visualizations via most used chart types and then derive insights 

## Welcome Message

Hi and welcome to the PowerBI Fundamentals course! This course is specially designed to offer a hands-on guided-learning approach that helps you meet your goals quickly, gain confidence, and learn at your own pace. 

## Summary Message

Congrats! You have successfully completed the first part of the PowerBI Fundamentals course! In the next parts we will further deep dive into developing your data-driven decision making skills!


### Section: Compare Consumentenomzet with bar charts
id: get-data
url: get-data

#### Exercise: Explore Consumentenomzet numbers ########################
id: types-getdata
url: types-getdata
type: video
video: https://vimeo.com/733216228/c24463d9e7


#### Exercise: Identify Assortimentscluster weekly changes
id: quiz-multichoice-demo
url: quiz-multichoice-demo
type: quiz
answer: 3

##### Content

<p>As an assortment manager at Jumbo, you should constantly analyze sales in order to compare and explain variations over different periods of time. Below you have the weekly revenue distribution (Consumentenomzet) by Assortimentcluster:</p>

<p align="middle"><img src="/static/RevenuePresentatieGroup.png" alt="recursion" align="middle" style="max-width: 100%"></p>


##### Task: Identify weekly changes
type: multichoice
answer: 2

###### Question

<p>By visually analyzing the revenue distribution above, which Assortimentcluster had the highest contribution (absolute value) to weekly increase from week 2022-14 to week 2022-15:</p>


###### Option #1

<p>Zuivel drink houdb</p>

####### Option Feedback

<p>Not quite, take a closer look again at the bar chart.</p>

###### Option #2

<p>Frisdranken</p>

####### Option Feedback

<p>That's correct! Frisdranken generated 0.5M week over week sales increase.</p>

###### Option #3

<p>Brood VVP/BED/ZAV</p>

####### Option Feedback

<p>Although Brood VVP/BED/ZAV had a strong contribution to the overall weekly growth, they did not had the highest contribution.</p>

###### Hint

<p>Compare the weekly Consumentenomzet, analyze each Assortimentcluster and identify the one which had the highest growth from week 2022-14 to 2022-15.</p>


#### Exercise: Drill-down on Presentatiegroep ###########
id: sales-by-month
url: sales-by-month
type: powerbi
video: https://vimeo.com/722307833/88e7653ce4


##### Context

<p>In the previous exercise you identified that Frisdranken Assortinmentcluster had the highest impact on weekly revenue increase from week 2022-14 to week 2022-15 </p>

<p>In this exercise you will further drill-down from Assortinmentcluster to Presentatiegroep level to identify and compare weekly increase and decrease.</p>


##### Instructions

###### Step #1

<p>Load the 2.1_WeeklyRevenue.pbx dashboard</p>

####### Step Hint 

<p>To open the workbook, click on File -> Open... in the upper menu</p>

###### Step #2

<p>Take a close look at the weekly revenue drill-down on Presentatiegroep for Frisdranken.</p>

<p>On the left and right part of the chart you have the total Revenue value for 2022-14 and 2022-15.</p>

####### Step Hint 

<p>Drag and drop the columns from the Data Pane in the left.</p>


##### Task
type: multichoice
answer: 1

###### Question

<p>Which Presentatiegroep contributed the most to the Frisdranken weekly sales increase?</p>

###### Option #1

<p>Cola</p>

####### Option Feedback

<p>Indeed, COLA generated 0.16M EUR weekly increase.</p>

###### Option #2

<p>Frisdrank</p>

####### Option Feedback

<p>Try again! Look closely and compare the bar charts.</p>

###### Option #3

<p>Water</p>

####### Option Feedback

<p>Yes, indeed! New customer are generating the highest profit!</p>

###### Hint

<p>Choose one of the provided options.</p>


#### Exercise: Identify weekly changes ########################
id: weekly-change
url: weekly-change
type: video
video: https://vimeo.com/654067950


### Section: Discover revenue trends over time
id: transform-data
url: transform-data

#### Exercise: Using Power Query###########################
id: power-query
url: power-query
type: video
video: https://vimeo.com/654067950

#### Exercise: Adding or removing columns###########################
id: transform-02
url: transform-02
type: video
video: https://vimeo.com/654067950

#### Exercise: Changing data types ###########################
id: change-data
url: change-data
type: video
video: https://vimeo.com/654067950

#### Exercise: Splitting columns ###########################
id: split-data
url: split-data
type: video
video: https://vimeo.com/654067950

#### Exercise: Other tasks ###########################
id: other-data
url: other-data
type: video
video: https://vimeo.com/654067950


### Section: Show product categories
id: clean-data
url: clean-data

#### Exercise: Handling missing values###########################
id: miss-values
url: miss-values
type: video
video: https://vimeo.com/654067950

#### Exercise: Removing duplicates###########################
id: remove-dupli
url: remove-dupli
type: video
video: https://vimeo.com/654067950

#### Exercise: Tips and tricks ###########################
id: tips-tricks
url: tips-tricks
type: video
video: https://vimeo.com/654067950


